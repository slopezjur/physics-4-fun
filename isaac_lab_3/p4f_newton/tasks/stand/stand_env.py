...
class StandEnv(DirectRLEnv):
    ...
    def __init__(self, cfg: StandEnvCfg, render_mode: str | None = None, **kwargs):
        ...
        self._thigh_ids = [self.robot.body_names.index(n) for n in ("Thigh_L", "Thigh_R")]
    ...
    def _apply_balance_assist(self) -> None:
        ...
        if self.cfg.balance_reaction:
            # **Into the THIGHS of planted legs, not the feet.**
            #
            # Godot reacts into the feet because it wants the torque to reach the ground, and Jolt
            # propagates it up the constrained chain to get there. Newton receives a raw body torque
            # instead, and a foot cannot hold one: measured inertia is 0.0009 kg.m^2 against the
            # pelvis's 0.0285, so the balance cap of 300 N.m spins a foot 663 DEGREES in a single
            # 120 Hz tick - far further than 2 XPBD iterations can pull back. The result was not a
            # hard task, it was divergence: rewards of -1e33 and NaN head heights.
            #
            # The thigh is also the more faithful partner. A pelvis attitude torque is generated by
            # the hip actuators, so the body it physically pushes against is the thigh; the feet are
            # a transmission path to the ground, not the source. At 0.0224 kg.m^2 the same torque
            # turns a thigh 27 degrees per tick - the same order as the pelvis it reacts against,
            # which is what the solver can actually integrate.
            #
            # Still gated on ground contact: an airborne leg has nothing to transmit into, and
            # loading it would just spin it.
            grounded = self._contacts()[:, 2:4]
            share = grounded / grounded.sum(dim=1, keepdim=True).clamp(min=1.0)
            for slot, body_id in enumerate(self._thigh_ids):
