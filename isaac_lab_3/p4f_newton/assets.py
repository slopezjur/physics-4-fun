...
# Per-joint scaling onto Godot's REAL actuator, stiffness and damping alike.
#
...
# `1 + kd*dt/I + kp*dt^2/I`. Measured live in the walk check at 120 Hz, body-wide:
#
#     authored kp 882  ->  effective 109        ceiling I/dt^2 = 579-701
#
# So Godot applies about a seventh of the gain the rig contract authors, while Isaac's XPBD applies
...
# itself apart. That shared denominator is also the useful part: Godot's actuator is the authored
# gains scaled DOWN, which is a plant Isaac reproduces exactly by scaling BOTH of its own the same
# way.
#
# **Two corrections to what this comment used to say.**
#
# 1. It is NOT a single factor. The denominator divides by each BONE's inertia, so the measured
#    spread is 34x - hand 0.0157, foot 0.0641, thigh 0.1757, chest 0.5404. The old "0.176 = 155/882"
#    is the THIGH's factor and nothing else's; applying it body-wide would leave the feet 2.7x too
#    stiff and the hands 11x too soft. The per-bone table now lives in the generated
#    `godot_plant.py`, regenerated by `scripts/derive_plant.py` from a Godot run log.
#
# 2. It used to be `float(os.environ.get("P4F_GAIN_SCALE", "1.0"))`, defaulting to OFF, and
#    `P4F_GAIN_SCALE` was set nowhere in the repository - so despite the twenty lines above calling
#    this the largest mismatch between the engines, the correction was never applied to a single
#    training run. An env-gated correction that defaults to inert is indistinguishable from no
#    correction at all. It is now applied unconditionally; `P4F_GAIN_SCALE` survives only as an extra
#    multiplier for sweeps, on top of the measured table.
GAIN_SCALE = float(os.environ.get("P4F_GAIN_SCALE", "1.0"))

# **The SPD factors alone do NOT make the two plants equal, and this is the correction that does.**
#
# `godot_plant.py` scales Isaac's gains by the factor Godot's Stable PD applies to its own authored
# ones. That is a real, measured discrepancy - but it silently assumes `ke` in Newton's XPBD drive
# and `Kp` in Godot's SPD buy the same torque. They do not: XPBD applies the drive as a compliant
# positional constraint INSIDE the solve, while Godot computes an explicit torque and applies it as
# a force.
#
# Measured by `scripts/probe_plant.py` against `Scenes/RL/Isaac3/Probe` - free root in both,
# gravity off, body settled, 120 Hz, step inside the joint limits on both sides.
#
# **On `joint_Thigh_L:1` (godot y, +/-0.5 rad, 0.15 rad step) - the DOF that actually carries a gait:**
#
#                              rise to 63.2% of command      overshoot
#     Godot                            33.3 ms                 +0.9%
#     Isaac, per-bone SPD scaling      33.3 ms                +10.5%     <- rise matches EXACTLY
#     Isaac, ke x 0.71                 50.0 ms                 +8.3%
#     Isaac, ke x 1.0, kd x 2.0        66.7 ms                 +4.3%
#
# So **the per-bone SPD table alone already equates the two plants' stiffness** and both calibration
# factors are 1.0. `ke` needs no correction at all.
#
# An earlier calibration of `ke` x 0.71 came from `joint_Forearm_L:0` and was WRONG. That joint's
# Godot axis has only ~+0.1 rad of positive headroom (it reaches -0.6 freely but clamps at +0.100,
# which disagrees with the rig's declared [-0.1, +2.6] and is an open question in its own right), so
# a 0.05 rad step sat at half the available travel and the response was shaped by limit proximity
# rather than by the drive. Applied body-wide it made the thigh 50% too slow. **Calibrate on a DOF
# with generous headroom in BOTH engines, and verify on a second one.**
#
# **Damping matches too, and the "structural gap" recorded here earlier was a probe artefact.**
# XPBD iterations are part of the plant, and the probe was reading the module default of 2 while
# training runs at 8. On `joint_Thigh_L:1`:
#
#     iterations 2    33.3 ms   +10.5% overshoot     <- what the earlier calibration measured
#     iterations 8    33.3 ms    +0.4% overshoot     <- what training actually uses
#     iterations 24   33.3 ms    +0.0% overshoot
#     Godot           33.3 ms    +0.9% overshoot
#
# So at the solver setting actually in use the two plants agree on BOTH rise time and overshoot, and
# nothing needs trading away. Rise time is what decides whether a swing leg reaches its commanded
# angle inside a step period; overshoot was never the problem it appeared to be. `probe_plant.py`
# now prints the iteration count for exactly this reason.
#
# Re-derive, do not guess, whenever the solver, its iteration count or the physics rate changes:
# run both probes and match rise time. `P4F_KE_CAL`/`P4F_KD_CAL` exist so that sweep needs no file
# edits.
XPBD_KE_CALIBRATION = float(os.environ.get("P4F_KE_CAL", "1.0"))
XPBD_KD_CALIBRATION = float(os.environ.get("P4F_KD_CAL", "1.0"))

if godot_plant.MEASURED_PHYSICS_HZ != 120:
    print(
        f"[assets] WARNING plant table measured at {godot_plant.MEASURED_PHYSICS_HZ} Hz, but the"
        " Godot contract runs 120 Hz physics. The SPD denominator is a function of dt, so these"
        " factors do not transfer across rates. Regenerate with scripts/derive_plant.py."
    )


def _scaled(gains: dict[str, float], calibration: float) -> dict[str, float]:
    """Apply each joint's measured Godot factor, then the solver calibration.

    An unmapped joint left at full gain is precisely the failure this table exists to prevent, and
    it would be invisible in `params/env.yaml` among 45 entries - so a miss raises here instead.
    """
    missing = [name for name in gains if godot_plant.scale_for_joint(name, 0.0) <= 0.0]
    if missing:
        raise RuntimeError(
            f"no Godot SPD factor for {len(missing)} joint(s): {sorted(missing)[:5]}. "
            "Regenerate p4f_newton/godot_plant.py with scripts/derive_plant.py."
        )
    return {
        name: value * godot_plant.scale_for_joint(name) * GAIN_SCALE * calibration
...
ACTUATOR_CFG = ImplicitActuatorCfg(
    joint_names_expr=[".*"],
    stiffness=_scaled(_stiffness, XPBD_KE_CALIBRATION),
    damping=_scaled(_damping, XPBD_KD_CALIBRATION),
