"""Fetch only the explicitly licensed neutral-idle source and preserve provenance."""
import argparse
import hashlib
import json
...
def verify_source(source: Path):
    for name, (_, _, expected) in FILES.items():
...
def acquire(out: Path):
    ...
    inventory = {}
    for name, (file_id, prefix, expected) in FILES.items():
    ...
    provenance = {"dataset": "100STYLE", "author": "Ian Mason", "source": PAGE,
                  ...
                  "credit": "The 100STYLE Dataset - Ian Mason", "verified_date": "2026-09-21",
